...
import (
	...
	"github.com/roshbhatia/gate/extras/internal/store"
...
` + "`serve`" + ` is the gate provider: on UserPromptSubmit it answers with the open
notes as context, so the model sees what the owner is waiting on.
`
