...
func pass(lines []string, found []fileAlert) (out []string, applied, unplaced int) {
	...
	for lineNo, list := range byLine {
		...
		// accident: a style names the more specific rule to sort first.
...
// `fix` writes the result back to a file; `check` records it for the model,
...
func fix(args []string) int {
	cfg, rest, err := cliConfig("prose-gate fix", args)
	if err != nil {
